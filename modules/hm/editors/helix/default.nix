...
# ──── LSP strategy: wrap hx with a buildEnv PATH prefix so   ───
# ──── all language servers are available without adding them ───
# ──── to home.packages or session PATH globally.            ───
{pkgs, ...}: let
  ...
  # ── Bundle all LSPs into a single PATH-able derivation ───────
  ...
  # ── Wrap hx to inject LSP PATH without polluting global PATH ─
  helixWithLSP =
